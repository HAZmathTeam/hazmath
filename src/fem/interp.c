...
/********************************************************************/
/*!
 * \fn void fe_interpolation_to_x(REAL* val,REAL* u,REAL* x,fe_local_data*
 * fe_data,simplex_local_data* elm_data,INT space_index)
 *
 ...
 *
 * \param u           Values of FEM approximation at DoF of space on element
 * only
 ...
 *
 */
void fe_interpolation_to_x(REAL* val, REAL* u, REAL* x, fe_local_data* fe_data,
                           simplex_local_data* elm_data, INT space_index) {
  INT i, j;
...
/*!
 * \fn void fe_interpolation_to_quadpt(REAL* val,fe_local_data*
 * fe_data,simplex_local_data* elm_data,INT space_index,INT quadpt)
 *
 * \brief Interpolate a finite-element approximation to a quadrature point in
 ...
 *
 */
void fe_interpolation_to_quadpt(REAL* val, fe_local_data* fe_data,
                                simplex_local_data* elm_data, INT space_index,
                                INT quadpt) {
  INT i, j;
...
/*!
 * \fn void fe_dinterp_to_x(REAL* val,REAL* u,REAL* x,fe_local_data*
 * fe_data,simplex_local_data* elm_data,INT space_index)
 *
 * \brief Interpolate the derivative of a finite-element approximation to any
 * other point in the given element using the given type of elements.
 *
 * \note We assume u is given and properly ordered on the given element
 ...
 *
 */
void fe_dinterp_to_x(REAL* val, REAL* u, REAL* x, fe_local_data* fe_data,
                     simplex_local_data* elm_data, INT space_index) {
  INT i, j, k;
  ...
  if ((fe_type == 20 && dim == 3) || (fe_type > 0 && fe_type < 20))
    grad_type = 2;
...
/*!
 * \fn void fe_dinterp_to_quadpt(REAL* val,fe_local_data*
 * fe_data,simplex_local_data* elm_data,INT space_index,INT quadpt)
 *
 * \brief Interpolate the derivative of a finite-element approximation to a
 * quadrature point in the given element using the given type of elements.
 *
 * \note Since the "type" of derivative depends on the FE type, we will have to
 *       check for various cases
 *
 ...
 * \param quadpt      Index of quadrature point in elment to interpolate to
 *
 * \return val        Pointer to value of derivative of approximation at given
 * quad point
 *
 */
void fe_dinterp_to_quadpt(REAL* val, fe_local_data* fe_data,
                          simplex_local_data* elm_data, INT space_index,
                          INT quadpt) {
  INT i, j, k;
...
/*!
 * \fn void blockfe_interpolation_to_x(REAL* val,REAL* u,REAL* x,fe_local_data*
 * fe_data,simplex_local_data* elm_data)
 *
 * \brief Interpolate a finite-element approximation to any other point in the
 *        given element for ALL FE spaces in system
 *
 * \note We assume u is given and properly ordered on the given element for each
 * space
 *
 * \param u           Values of FEM approximation at DoF of all spaces on
 * element only
 * \param x           Coordinates where to compute value
 * \param fe_data     Local FE space data
 * \param elm_data    Local Mesh Data
 *
 * \return val        Pointer to value of approximation at given x for all
 * spaces
 *
 */
void blockfe_interpolation_to_x(REAL* val, REAL* u, REAL* x,
...
/*!
 * \fn void blockfe_interpolation_to_quadpt(REAL* val,fe_local_data*
 * fe_data,simplex_local_data* elm_data,INT quadpt)
 *
 * \brief Interpolate a finite-element approximation to a quadrature point in
 * the given element using the given type of elements for ALL FE spaces in
 * system
 *
 ...
 * \param quadpt      Index of quadrature point in elment to interpolate to
 *
 * \return val        Pointer to value of approximation for all spaces at given
 * quadpt
 *
 */
void blockfe_interpolation_to_quadpt(REAL* val, fe_local_data* fe_data,
                                     simplex_local_data* elm_data, INT quadpt) {
  INT i;
...
/*!
 * \fn void blockfe_dinterp_to_x(REAL* val,REAL* u,REAL* x,fe_local_data*
 * fe_data,simplex_local_data* elm_data)
 *
 * \brief Interpolate derivative of a finite-element approximation to any other
 * point in the given element for ALL FE spaces in system
 *
 * \note We assume u is given and properly ordered on the given element for each
 * space
 *
 ...
 *
 */
void blockfe_dinterp_to_x(REAL* val, REAL* u, REAL* x, fe_local_data* fe_data,
                          simplex_local_data* elm_data) {
  INT i;
...
/*!
 * \fn void blockfe_dinterp_to_quadpt(REAL* val,fe_local_data*
 * fe_data,simplex_local_data* elm_data,INT quadpt)
 *
 * \brief Interpolate derivative of a finite-element approximation to a
 * quadrature point in the given element using the given type of elements for
 * ALL FE spaces in system
 *
 * \note We assume u is contained in fe_data for each space
 *
 * \param fe_data     Local FE space data
 ...
 * \param quadpt      Index of quadrature point in elment to interpolate to
 *
 * \return val        Pointer to derivative of approximation for all spaces at
 * given quadpt
 *
 */
void blockfe_dinterp_to_quadpt(REAL* val, fe_local_data* fe_data,
                               simplex_local_data* elm_data, INT quadpt) {
  INT i;
 ...
 *
 * \brief Evaluate a given analytical function on the specific degree of freedom
 * of the finite-element space given
 *
 ...
 *
 */
REAL FE_Evaluate_DOF(void (*expr)(REAL*, REAL*, REAL, void*), fespace* FE,
                     scomplex* sc, REAL time, INT DOF) {
  sc_fem* fem = sc->fem;
  INT dim = sc->dim;
  INT j, m;
  REAL x[dim + 1];
  ...
  INT elv;
  REAL dp1fact = 0.0;  // (dim+1)!
  REAL bubcoef = 0.0;  // (2dim+1)! / (d! |el_vol|)
  REAL expronv = 0.0;  // function on each vertex of element

  // P0 elements u[dof] = 1/elvol \int_el u
  if (FEtype == 0) {
    val = (1.0 / fem->el_vol[DOF]) *
          integrate_elm(expr, 1, 0, nq1d, NULL, sc, time, DOF);
    ...
    // linear components:
    // u[dof] = (int_f v*n_f - |f|/dim * sum(vertices on face) v(i)*n_f) / int_f
    // phi_f*n_f Note that since phi_f = 2^d lam_i lam_j ..., we have int_f
    // phi_f*n_f = 2^d (d-1)! / (2d-1)! |f|
  } else if (FEtype >= 60 && FEtype < 70) {
    valx = (REAL*)calloc(dim, sizeof(REAL));
    val = integrate_face_vector_normal(expr, dim, 0, nq1d, NULL, sc, time, DOF);
    get_incidence_row(DOF, fem->f_v, face_vertex);
    for (m = 0; m < dim; m++) {
      x[0] = sc->x[face_vertex[m] * dim];
      x[1] = sc->x[face_vertex[m] * dim + 1];
      if (dim == 3) x[2] = sc->x[face_vertex[m] * dim + 2];
      // The following only works for 2D
      (*expr)(valx, x, time, &(FE->dof_flag[DOF]));
      for (j = 0; j < dim; j++)
        val += -(fem->f_area[DOF] / dim) * fem->f_norm[DOF * dim + j] * valx[j];
    }
    // Divide by int_f \phi_f*n_f
    if (dim == 2) {  // In 2D, int_f \phi_f*n_f = 2/3 |f|
      val = 1.5 * val / (fem->f_area[DOF]);
    } else if (dim == 3) {  // In 3D, int_f \phi_f*n_f = 2/15 |f|
      val = 7.5 * val / (fem->f_area[DOF]);
    } else {
    ...
    // Element bubble routines (for now just MINI)
  } else if (FEtype == 103) {  // MINI
    // First check if the DOF is a vertex (linear) or element (bubble)
    if (DOF < nv) {  // Vertex DoF: val = f(vertex)
      for (j = 0; j < dim; j++) x[j] = FE->cdof->x[j * ndof + DOF];
      (*expr)(&val, x, time, &(FE->dof_flag[DOF]));
    } else {  // bubble DoF: val = int_elm (f) - sum_v_on_elm f(v)*int_elm
              // (lam_v)
      // Precompute some values:
      if (dim == 1) {
        dp1fact = 0.5 * fem->el_vol[DOF - nv];
        bubcoef = 6.0 / fem->el_vol[DOF - nv];
      } else if (dim == 2) {
        dp1fact = (1.0 / 3.0) * fem->el_vol[DOF - nv];
        bubcoef = 60.0 / fem->el_vol[DOF - nv];
      } else if (dim == 3) {
        dp1fact = 0.25 * fem->el_vol[DOF - nv];
        bubcoef = 840.0 / fem->el_vol[DOF - nv];
      } else {  // Not implmented above dim==3
        check_error(ERROR_DIM, __FUNCTION__);
      }
      // First get vertices on the element and evaluate function at each Vertex
      // and add up
      val = 0.0;
      get_incidence_row(DOF - nv, fem->el_v, loc_el_v);
      for (j = 0; j < (dim + 1); j++) {
        elv = loc_el_v[j];
        for (m = 0; m < dim; m++) x[m] = FE->cdof->x[m * ndof + elv];
        (*expr)(&expronv, x, time, &(sc->bndry[elv]));
        val += expronv * dp1fact;
      }
      // Integrate function over element and add
      val = bubcoef *
            (integrate_elm(expr, 1, 0, nq1d, NULL, sc, time, DOF - nv) - val);
    }
...
/****************************************************************************************************************************/
/*!
 * \fn void FE_Evaluate(REAL* val,void (*expr)(REAL *,REAL *,REAL,void
 * *),fespace *FE,scomplex *sc,REAL time)
 *
 * \brief Evaluate (Project) a given function (analytic expression) on the given
 ...
 *
 */
void FE_Evaluate(REAL* val, void (*expr)(REAL*, REAL*, REAL, void*),
                 fespace* FE, scomplex* sc, REAL time) {
  INT i;

  for (i = 0; i < FE->ndof; i++) {
    val[i] = FE_Evaluate_DOF(expr, FE, sc, time, i);
  }
...
/****************************************************************************************************************************/
/*!
 * \fn REAL blockFE_Evaluate_DOF(void (*expr)(REAL *,REAL *,REAL,void
 * *),block_fespace *FE,scomplex *sc,REAL time,INT comp,INT DOF)
 *
 * \brief Evaluate a given analytical function on the specific degree of freedom
 * of the block finite-element space given
 *
 * \param expr 	    Function call to analytical expression
 * \param FE        FE Space
 * \param mesh      Mesh Data
 ...
 *
 */
REAL blockFE_Evaluate_DOF(void (*expr)(REAL*, REAL*, REAL, void*),
                          block_fespace* FE, scomplex* sc, REAL time, INT comp,
                          INT DOF) {
  sc_fem* fem = sc->fem;
  INT dim = sc->dim;
  INT i, j, m;
  ...
  REAL dp1fact = 0.0;               // (dim+1)!
  REAL bubcoef = 0.0;               // (2dim+1)! / (d! |el_vol|)
  REAL expronv[dim * FE->nspaces];  // function on each vertex of element

  for (i = 0; i < comp; i++) {
    if (FE->var_spaces[i]->scal_or_vec == 0 &&
        FE->var_spaces[i]->FEtype != 61) {
      local_dim += 1;
    } else if (FE->var_spaces[i]->FEtype == 61) {  // Bubble Element
      local_dim += 0;  // Assuming bubbles are written such that linears follow
                       // it immediately
    } else {           // Vector Element
      local_dim += dim;
    }
  ...
  if (FEtype == 0) {
    val = (1.0 / fem->el_vol[DOF]) *
          integrate_elm(expr, FE->nun, local_dim, nq1d, NULL, sc, time, DOF);
    ...
    val = integrate_face_vector_normal(expr, FE->nun, local_dim, nq1d, NULL, sc,
                                       time, DOF);
    get_incidence_row(DOF, fem->f_v, face_vertex);
    for (m = 0; m < dim; m++) {
      x[0] = sc->x[face_vertex[m] * dim];
      ...
      (*expr)(valx, x, time, &(FE->var_spaces[comp]->dof_flag[DOF]));
      for (j = 0; j < dim; j++)
        val += -(fem->f_area[DOF] / dim) * fem->f_norm[DOF * dim + j] *
               valx[local_dim + j];
    }
    // Divide by int_f \phi_f*n_f
    if (dim == 2) {  // In 2D, int_f \phi_f*n_f = 2/3 |f|
    ...
    if (DOF < nv) {  // Vertex DoF: val = f(vertex)
      ...
      for (j = 0; j < (dim + 1); j++) {
        elv = loc_el_v[j];
        for (m = 0; m < dim; m++)
          x[m] = FE->var_spaces[comp]->cdof->x[m * ndof + elv];
        (*expr)(expronv, x, time, &(sc->bndry[elv]));
        val += expronv[local_dim] * dp1fact;
      }
      // Integrate function over element and add
      val = bubcoef * (integrate_elm(expr, FE->nun, local_dim, nq1d, NULL, sc,
                                     time, DOF - nv) -
                       val);
    }
    // No other FEM types implemented
...
/****************************************************************************************************************************/
/*!
 * \fn void blockFE_Evaluate(REAL* val,void (*expr)(REAL *,REAL *,REAL,void
 * *),block_fespace *FE,scomplex *sc,REAL time)
 *
 ...
 *
 */
void blockFE_Evaluate(REAL* val, void (*expr)(REAL*, REAL*, REAL, void*),
                      block_fespace* FE, scomplex* sc, REAL time) {
  int i, k;
  INT entry = 0;
  ...
  for (k = 0; k < FE->nspaces; k++) {
    for (i = 0; i < FE->var_spaces[k]->ndof; i++) {
      val[entry] = blockFE_Evaluate_DOF(expr, FE, sc, time, k, i);
      entry++;
    }
  }
...
/****************************************************************************************************************************/
/*!
 * \fn void blockFE_EvaluateBoundary(REAL* val,void (*expr)(REAL *,REAL
 * *,REAL,void *),block_fespace *FE,scomplex *sc,REAL time)
 *
 * \brief Evaluate (Project) a given analytical function on the block
 ...
 *
 */
void blockFE_EvaluateBoundary(REAL* val,
                              void (*expr)(REAL*, REAL*, REAL, void*),
                              block_fespace* FE, scomplex* sc, REAL time) {
  int i, k;
  INT entry = 0;
...
/***********************************************************************************************/
/*!
 * \fn void Project_to_Vertices(REAL* u_on_V,REAL *u,fespace *FE,scomplex *sc)
 *
...
/****************************************************************************************************************************/
/*!
 * \fn void get_unknown_component(dvector* u,dvector* ublock,block_fespace
 * *FE,INT comp)
 *
 * \brief Grab a component of a block FE function.
 *
 * \param u	         FE approximation of function on specific block of
 * fespace
 ...
 *
 */
void get_unknown_component(dvector* u, dvector* ublock, block_fespace* FE,
                           INT comp) {
  INT i = 0;
  INT entry = 0;
...
/****************************************************************************************************************************/
/*!
 * \fn void set_unknown_component(dvector* u,dvector* ublock,block_fespace
 * *FE,INT comp)
 *
 ...
 *
 */
void set_unknown_component(dvector* u, dvector* ublock, block_fespace* FE,
                           INT comp) {
  INT i = 0;
  INT entry = 0;
...
/***********************************************************************************************/
/*!
 * \fn void get_grad_H1toNed(dCSRmat* Grad,scomplex* sc)
 *
 * \brief Computes Gradient operator.
 *        Applying the resulting matrix computes the gradient of an H1
 * approximation. Takes Nodal (H1) DOF vector to Edge (Nedelec) DOF vector
 *        Ordering determined by edge to node map: bigger node is +1 smaller is
 * -1
 *
 * \param Grad	     dCSRmat matrix that takes the gradient of an H1
 * approximation
 * \param mesh       Mesh data
 *
...
void get_grad_H1toNed(dCSRmat* Grad, scomplex* sc) {
  ...
  Gtmp.row = fem->ed_v->row;
  Gtmp.col = fem->ed_v->col;
  Gtmp.nnz = fem->ed_v->nnz;
  Gtmp.IA = (INT*)calloc(Gtmp.row + 1, sizeof(INT));
  Gtmp.JA = (INT*)calloc(Gtmp.nnz, sizeof(INT));
  Gtmp.val = (REAL*)calloc(Gtmp.nnz, sizeof(REAL));

  for (i = 0; i <= nedge; i++) {
    Gtmp.IA[i] = fem->ed_v->IA[i];
  }
  for (i = 0; i < Gtmp.nnz; i++) {
    Gtmp.JA[i] = fem->ed_v->JA[i];
  }

  for (i = 0; i < nedge; i++) {
    oneoverlen = 1.0 / (fem->ed_len[i]);
    rowa = fem->ed_v->IA[i];
    j = fem->ed_v->JA[rowa];
    k = fem->ed_v->JA[rowa + 1];
    if (j > k) {
...
/***********************************************************************************************/
/*!
 * \fn void get_curl_NedtoRT(dCSRmat* Curl,scomplex* sc)
 *
 * \brief Computes Curl operator in 3D ONLY.
 *        Applying the resulting matrix computes the Curl of a Nedelec
 * approximation. Takes Edge (Nedelec) DOF vector to Face (RT) DOF vector
 *
 * \param Curl 	     dCSRmat matrix that takes the curl of a Nedelec
 * approximation
 * \param mesh         Mesh data
 *
 ...
 *
 */
void get_curl_NedtoRT(dCSRmat* Curl, scomplex* sc) {
  sc_fem* fem = sc->fem;
  INT dim = sc->dim;
  ...
  Ktmp.row = fem->f_ed->row;
  Ktmp.col = fem->f_ed->col;
  Ktmp.nnz = fem->f_ed->nnz;
  ...
  for (i = 0; i <= Ktmp.row; i++) {
    Ktmp.IA[i] = fem->f_ed->IA[i];
  }
  for (i = 0; i < Ktmp.nnz; i++) {
    Ktmp.JA[i] = fem->f_ed->JA[i];
  }

  // Get Kcurl -> if_ed,jf_ed, sign = sign(det(xi-xk,xj-xk,n_fijk))
  for (i = 0; i < nface; i++) {
    // Get Normal Vector
    vec3[0] = fem->f_norm[i * mydim];
    vec3[1] = fem->f_norm[i * mydim + 1];
    vec3[2] = fem->f_norm[i * mydim + 2];
    // Get nodes of Face
    rowa = fem->f_v->IA[i];
    rowb = fem->f_v->IA[i + 1];
    jcntr = 0;
    for (j = rowa; j < rowb; j++) {
      inf[jcntr] = fem->f_v->JA[j];
      jcntr++;
    }
    // Get edges of face
    rowa = fem->f_ed->IA[i];
    rowb = fem->f_ed->IA[i + 1];
    for (j = rowa; j < rowb; j++) {
      k = fem->f_ed->JA[j];
      // Get nodes of edge
      col_a = fem->ed_v->IA[k];
      nd1 = fem->ed_v->JA[col_a];
      nd2 = fem->ed_v->JA[col_a + 1];
      ...
      vec1[0] = sc->x[nd1 * dim] - sc->x[nd3 * dim];
      vec2[0] = sc->x[nd2 * dim] - sc->x[nd3 * dim];
      vec1[1] = sc->x[nd1 * dim + 1] - sc->x[nd3 * dim + 1];
      vec2[1] = sc->x[nd2 * dim + 1] - sc->x[nd3 * dim + 1];
      vec1[2] = sc->x[nd1 * dim + 2] - sc->x[nd3 * dim + 2];
      vec2[2] = sc->x[nd2 * dim + 2] - sc->x[nd3 * dim + 2];
      if (nd1 > nd2) {
  ...
  for (i = 0; i < nface; i++) {
    ...
    for (j = rowa; j < rowb; j++) {
      k = fem->f_ed->JA[j];
      Ktmp.val[j] =
          (1.0 / (fem->f_area[i])) * ((REAL)Ktmp.val[j]) * (fem->ed_len[k]);
    }
  }
...
/***********************************************************************************************/
/*!
 * \fn void get_div_RTtoL2(dCSRmat* Div,scomplex* sc)
 *
 * \brief Computes Divergence operator.
 *        Applying the resulting matrix computes the divergence of an RT
 * approximation. Takes Face (RT) DOF vector to Volume (L2) DOF vector. Ordering
 * determined by element to face map: orientation determined by normal vector of
 * face. Normal vectors point from lower number element to higher one or outward
 * from external boundary Entry gets positive 1 if normal is outward of element
 * and -1 if inward of element
 *
 * \param Div 	     dCSRmat matrix that takes the div of a RT approximation
 * \param mesh         Mesh data
 *
 * \note This only makes sense for lowest order elements...I think...and 2D or
 * 3D
 *
 */
void get_div_RTtoL2(dCSRmat* Div, scomplex* sc) {
  ...
  Dtmp.row = fem->el_f->row;
  Dtmp.col = fem->el_f->col;
  Dtmp.nnz = fem->el_f->nnz;
  Dtmp.IA = (INT*)calloc(Dtmp.row + 1, sizeof(INT));
  Dtmp.JA = (INT*)calloc(Dtmp.nnz, sizeof(INT));
  Dtmp.val = (REAL*)calloc(Dtmp.nnz, sizeof(REAL));

  for (i = 0; i <= nelm; i++) {
    Dtmp.IA[i] = fem->el_f->IA[i];
  }
  for (i = 0; i < Dtmp.nnz; i++) {
    Dtmp.JA[i] = fem->el_f->JA[i];
  }

  for (i = 0; i < nelm; i++) {
    oneovervol = 1.0 / (fem->el_vol[i]);
    // Get faces of element
    rowa = fem->el_f->IA[i];
    rowb = fem->el_f->IA[i + 1];
    for (j = rowa; j < rowb; j++) {
      face = fem->el_f->JA[j];
      farea = fem->f_area[face];
...
/***********************************************************************************************/
/*!
 * \fn void get_Pigrad_H1toNed(dCSRmat* Pgrad,scomplex* sc)
 *
 * \brief Computes Gradient operator into scalar components for HX
 * preconditioner. Ordering determined by edge to node map: bigger node is +1
 * smaller is -1
 *
 * \param Pgrad 	     dCSRmat matrix that takes the the \Pi for HX
 * preconditioner.
 * \param mesh         Mesh data
 *
 ...
 *
 */
void get_Pigrad_H1toNed(dCSRmat* Pgrad, scomplex* sc) {
  sc_fem* fem = sc->fem;
  INT dim = sc->dim;
  INT i, j, k, rowa, cola;
  INT nedge = fem->nedge;
  ...
  Ptmp.row = fem->ed_v->row;
  Ptmp.col = (fem->ed_v->col) * dim;
  Ptmp.nnz = (fem->ed_v->nnz) * dim;
  ...
  for (i = 0; i < nedge; i++) {
    ...
    k = fem->ed_v->JA[rowa + 1];
    if (j > k) {
      xL = 0.5 * oneoverlen * ((sc->x[j * dim]) - (sc->x[k * dim]));
      yL = 0.5 * oneoverlen * ((sc->x[j * dim + 1]) - (sc->x[k * dim + 1]));
      if (dim == 3)
        zL = 0.5 * oneoverlen * ((sc->x[j * dim + 2]) - (sc->x[k * dim + 2]));
    } else {
      xL = 0.5 * oneoverlen * ((sc->x[k * dim]) - (sc->x[j * dim]));
      yL = 0.5 * oneoverlen * ((sc->x[k * dim + 1]) - (sc->x[j * dim + 1]));
      if (dim == 3)
        zL = 0.5 * oneoverlen * ((sc->x[k * dim + 2]) - (sc->x[j * dim + 2]));
    }
    Ptmp.JA[cola] = j * dim;
    Ptmp.val[cola] = xL;
...
/***********************************************************************************************/
/*!
 * \fn void get_Pigrad_H1toRT( dCSRmat* Pdiv, dCSRmat* Pcurl, dCSRmat* Curl,
 * scomplex* sc)
 *
 ...
 */
/***********************************************************************************************/
void get_Pigrad_H1toRT(dCSRmat* Pdiv, dCSRmat* Pcurl, dCSRmat* Curl,
                       scomplex* sc) {
  sc_fem* fem = sc->fem;
  INT dim = sc->dim;
  INT i, j, rowa, cola;
  ...
  Ptmp.row = fem->f_v->row;
  Ptmp.col = (fem->f_v->col) * dim;
  Ptmp.nnz = (fem->f_v->nnz) * dim;
  Ptmp.IA = (INT*)calloc(Ptmp.row + 1, sizeof(INT));
  Ptmp.JA = (INT*)calloc(Ptmp.nnz, sizeof(INT));
  Ptmp.val = (REAL*)calloc(Ptmp.nnz, sizeof(REAL));
  ...
  for (i = 0; i < nedge; i++) {
    rowa = fem->ed_v->IA[i];
    v1 = fem->ed_v->JA[rowa];
    v2 = fem->ed_v->JA[rowa + 1];
    cola = Pcurl->IA[i];
    PcurlY[i] = (sc->x[v1 * dim + 1]) * (Pcurl->val[cola]) +
                (sc->x[v2 * dim + 1]) * (Pcurl->val[cola + dim]);
    PcurlZ[i] = (sc->x[v1 * dim + 2]) * (Pcurl->val[cola + 1]) +
                (sc->x[v2 * dim + 2]) * (Pcurl->val[cola + dim + 1]);
    PcurlX[i] = (sc->x[v1 * dim]) * (Pcurl->val[cola + 2]) +
                (sc->x[v2 * dim]) * (Pcurl->val[cola + dim + 2]);

    //    // PiV1 * y
    //    cola = Pcurl->IA[i]-1;
    //    j = Pcurl->JA[cola+1]-1;
    //    k = Pcurl->JA[cola+dim+1]-1;
    //    PcurlY[i] = (sc->x[v1*dim+1])*(Pcurl->val[j]) +
    //    (sc->x[v2*dim+1])*(Pcurl->val[k]);
    //    // PiV2 * z
    //    cola = Pcurl->IA[i]-1;
    //    j = Pcurl->JA[cola+2]-1;
    //    k = Pcurl->JA[cola+dim+2]-1;
    //    PcurlZ[i] = (sc->x[v1*dim+2])*(Pcurl->val[j]) +
    //    (sc->x[v2*dim+2])*(Pcurl->val[k]);
    //    // PiV3 * x
    //    cola = Pcurl->IA[i]-1;
    //    j = Pcurl->JA[cola]-1;
    //    k = Pcurl->JA[cola+dim]-1;
    //    PcurlX[i] = (sc->x[v1*dim])*(Pcurl->val[j]) +
    //    (sc->x[v2*dim])*(Pcurl->val[k]);
  }
  ...
  for (i = 0; i < nface; i++) {
    ...
    temp2 = temp2 / 3;
    temp3 = temp3 / 3;
    //    temp1 = temp1 / fem->f_area[i];
    //    temp2 = temp2 / fem->f_area[i];
    //    temp3 = temp3 / fem->f_area[i];

    // printf("F_norm(%f,%f,%f)\ttemp(%f,%f,%f)\n",fem->f_norm[i*dim],fem->f_norm[i*dim+1],fem->f_norm[i*dim+2],temp1,temp2,temp3);

    // temp1 = fem->f_area[i]*fem->f_norm[i*dim+0]/3;
    // temp2 = fem->f_area[i]*fem->f_norm[i*dim+1]/3;
    // temp3 = fem->f_area[i]*fem->f_norm[i*dim+2]/3;
    temp1 = fem->f_norm[i * dim + 0] / 3;
    temp2 = fem->f_norm[i * dim + 1] / 3;
    temp3 = fem->f_norm[i * dim + 2] / 3;
    ...
    v1 = fem->f_v->JA[rowa];
    v2 = fem->f_v->JA[rowa + 1];
    v3 = fem->f_v->JA[rowa + 2];
