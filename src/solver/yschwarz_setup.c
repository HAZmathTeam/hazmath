...
#include "hazmath.h"
static void ySchwarz_levels (INT, dCSRmat *, INT *, INT *, INT *, INT *, INT);
...
/***********************************************************************************************/
/**
 * \fn void ySchwarz_get_block_matrix (Schwarz_data *Schwarz, INT nblk,
 ...
 *
 */
static void ySchwarz_get_block_matrix (Schwarz_data *Schwarz,
				       INT nblk,
				       INT *iblock,
				       INT *jblock,
				       INT *mask)
...
/***********************************************************************************************/
/**
 * \fn INT ySchwarz_setup (Schwarz_data *Schwarz, Schwarz_param *param)
 *
 ...
 *
 */
INT ySchwarz_setup(Schwarz_data *Schwarz,
		   Schwarz_param *param)
{
  // information about A
  dCSRmat A = Schwarz->A;
  ...
  for ( i = 0; i < MaxIndSet->row; i++ ) {
    ...
    ySchwarz_levels(inroot,&A,mask,&nlvl,maxa,jblock,maxlev);
    nsizei=maxa[nlvl];
  ...
  for (i=0;i<MaxIndSet->row;i++) {
    inroot = MaxIndSet->val[i];
    ySchwarz_levels(inroot,&A,mask,&nlvl,maxa,jb,maxlev);
    nsizei=maxa[nlvl];
  ...
  ySchwarz_get_block_matrix(Schwarz, nblk, iblock, jblock, mask);
...
/***********************************************************************************************/
/**
 * \fn INT ySchwarz_setup_with_seeds(Schwarz_data *Schwarz, Schwarz_param *param, ivector *seeds)
 *
 ...
 *
 */
INT ySchwarz_setup_with_seeds(Schwarz_data *Schwarz,
			      Schwarz_param *param,
			      ivector *seeds)
{
  // information about A
...
/**
 * \fn static void ySchwarz_levels (INT inroot, dCSRmat *A, INT *mask, INT *nlvl,
 ...
 *
 */
static void ySchwarz_levels (INT inroot,
                            dCSRmat *A,
                            INT *mask,
                            INT *nlvl,
                            INT *iblock,
