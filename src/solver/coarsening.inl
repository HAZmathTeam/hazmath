                              ...
                              AMG_param *param )
{
	...
	S->IA = (INT *)calloc(row1, sizeof(INT));
	S->JA = (INT *)calloc(nnz,  sizeof(INT));
	iarray_cp(row1, ia, S->IA);
	iarray_cp(nnz,  ja, S->JA);
                             ...
                             ivector *vertices)
{
	...
	Stemp.IA = (INT *)calloc(S->row+1, sizeof(INT));
	Stemp.JA = (INT *)calloc(S->nnz,   sizeof(INT));
	iarray_cp(S->row+1, S->IA, Stemp.IA);
	iarray_cp(S->nnz,   S->JA, Stemp.JA);
	...
	free(S->IA); S->IA = Stemp.IA;
	free(S->JA); S->JA = Stemp.JA;
                                   ...
                                   ivector *CGPT_rindex)
{
	...
	Sh->row = Sh->col = num_c;
	Sh->val = Sh->JA = NULL;
	Sh->IA  = (INT*)calloc(Sh->row+1, sizeof(INT));
	
	// record the number of times some coarse point is visited
	visited = (INT*)calloc(num_c, sizeof(INT));
	iarray_set(num_c, visited, -1);
	...
	iarray_set(num_c, visited, -1); // reset visited
	
	Sh->nnz = Sh->IA[Sh->row];
	Sh->JA  = (INT*)calloc(Sh->nnz, sizeof(INT));
                            ...
                            INT aggressive_path)
{
    ...
    ivec_free(&CGPT_index);
    ivec_free(&CGPT_rindex);
    icsr_free(&Sh);
    icsr_free(&ST);
    icsr_free(&ShT);
    free(work);
                                ...
                                INT col )
{
    ...
    // Initialize P matrix
    P->row = row; P->col = col;
    P->IA  = (INT*)calloc(row+1, sizeof(INT));
    ...
    P->val = (REAL*)calloc(P->nnz,sizeof(REAL));
                                ...
                                INT col)
{
	...
	iarray_set(row, visited, -1); // re-init visited array
                       ...
                       AMG_param *param)
{
    ...
    /**
     * \fn SHORT amg_coarsening_c (dCSRmat *A, ivector *vertices, dCSRmat *P,
    ...
    switch ( coarse_type ) {
        ...
        case COARSE_C: // Classical coarsening
            col = cfsplitting_cls(A, S, vertices); break;
            
        case COARSE_CP: // Classical coarsening with positive connections
