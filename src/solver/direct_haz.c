...
/**
 * \fn static void solve_factored_symm(dCSRmat *U,dvector *dinv, dvector *x)
 *
 * \brief Reduction and back substitution for solving the symmetric
 *        system with decomposed (factorized matrix)
 *
 ...
 *
 */
static void solve_factored_symm(dCSRmat *U,dvector *dinv, dvector *x)
{
  /*
    --------------------------------------------------------------------
    ...  REDUCTION AND BACK SUBSTITUTION FOR SOLVING THE
    ...  SYMMETRIC SYSTEM WITH DECOMPOSED (FACTORIZED MATRIX)
    --------------------------------------------------------------------
  */
...
static void symbolic_factor_symm(dCSRmat *AU, dCSRmat *U)
{
  ...
  */
  ...
  //
  //  fprintf(stdout,"\nlast_u=%d (rows=%d,nnzu=%d)\n",last_u,U->row,U->nnz);fflush(stdout);
  INT nm1 = n - 1;
  for(i=0;i<n;i++){
    U->IA[i] = -1;
    ichn[i] = -1;
...
/**************************************************************************/
/**
 * \fn void* hazmath_factorize(dCSRmat *A,INT print_level)
 *
 ...
 *
 */
void *hazmath_factorize(dCSRmat *A,INT print_level)
{
  ...
  wrk += 3*sizeof(SHORT);
  // end alloc for the NNumeric structure. 
  /**/
  dCSRmat AU,AL;
  dvector adiag;
  n=A->row; nnz=A->nnz;
  ...
  AL.JA=realloc(AL.JA,(AL.nnz)*sizeof(INT));
  //SHOOULD BE NULL:  AL.val=NULL;//realloc(AL.val,(AL.nnz)*sizeof(REAL));
  // order U:
  U->val=NULL;
...
/********************************************************************/
/**
 * \fn INT hazmath_solve(dCSRmat *A,dvector *f,dvector *x, void* Numeric, INT print_level)
 *
 * \brief Performs Gaussian Elmination on Ax = f, using HAZMATH
 *        (Assumes A is symmetric and in CSR format and that the
 ...
 *
 */
INT hazmath_solve(dCSRmat *A,dvector *f,dvector *x, \
		  void* Numeric, INT print_level)
{
  // get all the factor(s):
  void *wrk=(void *)Numeric;
  dCSRmat *U=(dCSRmat *)wrk;
  wrk+=sizeof(dCSRmat);
  dvector *dinv=(dvector *)wrk;
  wrk+=sizeof(dvector);
  // SHORT *extra=wrk;// unused
  // end of the *Numeric array/pointer/structure:
  wrk+=3*sizeof(SHORT);
  //now only symetric matrices:  SHORT is_sym=extra[0];
  //  
  x->row=f->row;
  memcpy(x->val,f->val,x->row*sizeof(REAL));
  solve_factored_symm(U,dinv,x);
  return (INT )SUCCESS; 
}
/********************************************************************/
/**
 ...
 *
 * \brief clean up the direct solver structure Numeric. 
 *
 ...
 * \date   20220802
 */
void hazmath_free_numeric(void *Numeric)
{
  void *wrk=(void *)Numeric;
  dCSRmat *U=(dCSRmat *)wrk;
  wrk+=sizeof(dCSRmat);
