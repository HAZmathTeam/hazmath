 ...
 *
 * \param xstar[]:   node for which we would like to check its incidence with
 * the simplex.
 *
...
INT xins(INT n, INT* nodes, REAL* xs, REAL* xstar) {
  ...
  //  fflush(stdout);
  for (i = 0; i < n; i++) xhat[i] = xstar[i] - xs[l0n + i];
  ...
  for (i = 0; i < ns; i++) {
    ...
    for (j = 0; j < n; j++) {
      ...
      for (k = j1; k < n1; k++) {
        ...
        for (p = 0; p < nbig; p++) {
          sl[ke] +=
              (sc->x[mj + p] - sc->x[mk + p]) * (sc->x[mj + p] - sc->x[mk + p]);
        }
        sl[ke] = sqrt(sl[ke]);
        ke++;
      }
 ...
 *
 */
unsigned INT reflect2(INT n, INT is, INT it, INT* sv1, INT* sv2, INT* stos1,
                      INT* stos2, INT visited, INT* wrk)
/*************************************************************/
    ...
    for (i = 0; i < n1; i++) {
      if (stos1[i] != it) {
        if (sv1[i] - sv2[i]) {
          ...
          fprintf(stderr,
                  "\n***ERROR in %s ; (is)=%lld(vis=%lld) and (it) = %lld are "
                  "both visited but are not reflected neighbors.\n\n",
                  __FUNCTION__, (long long)is, (long long)visited,
                  (long long)it);
          fprintf(stderr,
...
void abfstree(const INT it0, scomplex* sc, INT* wrk, const INT print_level) {
  ...
  for (kcc = 0; kcc < cc; kcc++) {
    ...
    while (1) {
      ...
      if (kend >= nums)
        break; /* exit here even if we have not visited all simplices as they
  ...
  */
  ...
  for (j = 0; j < sc->ns; j++) {
    if (sc->child0[j] < 0 || sc->childn[j] < 0) {
      ...
      sctop->marked[ns] = sc->marked[j];  // making sure nothing is marked on
                                          // the top for refinement;
      sctop->vols[ns] = sc->vols[j];  // making sure nothing is marked on the
                                      // top for refinement;
      ns++;
    }
  }
 ...
 *
 * \note This is a modified version of the real scfinalize avoiding some memory
 * issues with the boundary codes until that is fixed.
 *
 */
void scfinalize_nofree(scomplex* sc, const INT set_bndry_codes) {
  ...
  */
  ...
  /*   } */
  /* } */
  // clean up: TODO: DO NOT FREE ANYTHING UNTIL LUDMIL FIXES! JHA: we actually
  // think this is right and should update scfinalize to this
 ...
 *
 */
static unsigned INT bitdiff(const INT dim, unsigned INT* bits1,
                            unsigned INT* bits2) {
  INT j;
...
void find_cc_bndry_cc(scomplex* sc, const INT set_bndry_codes) {
  ...
  if (nbf != nbfnew) {
    fprintf(stderr,
            "\n%%***ERROR(1): num. bndry faces mismatch (nbf=%lld .ne. "
            "nbfnew=%lld) in %s",
  ...
  if (f2v.col < sc->nv) indxinv = realloc(indxinv, f2v.col * sizeof(INT));
  for (i = 0; i < f2v.nnz; ++i) f2v.JA[i] = indx[f2v.JA[i]];
  ...
  */
  ...
  // a vertex may lie the intersection of many boundaries as the dimension of
  // the problem increases. sc_bndry_v is an iCSRmat that for every vertex, it
  // contains all boundaries it lies on.
  //
  ...
  if (TRUE) {
    ...
    sc->bndry_v->IA =
        realloc(sc->bndry_v->IA, (sc->parent_v->row + 1) * sizeof(INT));
    ...
    if (nnz_bv) {
      ...
      for (k = 0; k < sc->parent_v->row; ++k) {
        ...
        if ((sc->parent_v->IA[k + 1] - j) == 2) {
          ...
          // fprintf(stdout,"\nnew_vertex=%d,v1=%d,v2=%d;
          // n1=%d,n2=%d",k,v1,v2,n1,n2);fflush(stdout);
          ...
          if (ncap) {
            ...
            for (i = 0; i < ncap; ++i) {
              ...
              sc->bndry_v->val[nnz_bv + i + nnzold] =
                  sc->bndry_v->val[l + nnzold];
            }
            nnz_bv += ncap;
          }
  ...
  if (!nnz_bv) {
    ...
    for (i = 0; i < sc->bndry_cc; ++i) {
      for (k = blk_dfs->IA[i]; k < blk_dfs->IA[i + 1]; ++k) {
        ...
        for (m = 0; m < dim; m++) {
          // fprintf(stdout,"\nBV=%d,code=%d;
          // face=%d",indxinv[f2v.JA[dim*j+m]],i,k);
          sc->bndry[indxinv[f2v.JA[dim * j + m]]] = i + 1;
        }
      }
    }
    sc->bndry_v->IA =
        realloc(sc->bndry_v->IA, (sc->bndry_v->row + 1) * sizeof(INT));
    sc->bndry_v->JA =
        realloc(sc->bndry_v->JA, dim * (sc->bndry_v->row) * sizeof(INT));
    sc->bndry_v->val =
        realloc(sc->bndry_v->val, dim * (sc->bndry_v->row + 1) * sizeof(INT));
  ...
  for (i = 1; i < sc->bndry_v->nnz; ++i) {
    if (cmin > sc->bndry_v->val[i]) cmin = sc->bndry_v->val[i];
    if (cmax < sc->bndry_v->val[i]) cmax = sc->bndry_v->val[i];
  }
  cmin--;
  cmax++;
  fprintf(stdout, "cmin=%d,cmax=%d,nnz_bv=%d,nnz_in bv=%d,k123=%d,rows:%10d\n",
          cmin, cmax, nnz_bv, sc->bndry_v->nnz,
          sc->bndry_v->IA[sc->bndry_v->row], sc->bndry_v->row);
  fflush(stdout);
  exit(2);
  if (!cmin) cmin = -1;
  if (!cmax) cmax = 1;
...
void mapit(scomplex* sc, REAL* vc) {
  ...
  for (kf = 0; kf < sc->nv; kf++) {
    ...
    for (i = 0; i < dim; i++)
      sc->x[kf * dim + i] = interp4(c2s, vcp + i * c2s->nvcube, xhat);
  }
  cube2simp_free(c2s);
  free(vcp_xhat);
  return;
...
void vtkw(const char* namevtk, vtu_data* vdata) {
  ...
  */
  ...
  fprintf(fvtk, "<Points>\n");
  fprintf(fvtk,
          "<DataArray type=\"%s\" NumberOfComponents=\"3\" Format=\"ascii\">",
          tfloat);
  for (j = 0; j < nv; j++) {
    for (k = 0; k < nbig; k++) {
  ...
  fprintf(fvtk, "</Points>\n");
  /*NOT USED: if(sc->fval){ */
  /*   fprintf(fvtk,"<DataArray type=\"%s\" Name=\"ele\"
   * Format=\"ascii\">",tfloat); */
  ...
  // region and boundary.
  /* if(sc->bndry_cc>1) { */
  /*   fprintf(fvtk,"<DataArray type=\"%s\" Name=\"connectedcomponents\"
   * Format=\"ascii\">",tinto); */
  /*   for(k=0;k<nv;k++) { */
  /*     if(ib[k]==0) { */
  ...
  fprintf(fvtk, "<Cells>\n");
  fprintf(fvtk, "<DataArray type=\"%s\" Name=\"offsets\" Format=\"ascii\">",
          tinto);
  for (k = 1; k <= ns; k++) fprintf(fvtk, " %lld ", (long long)(k * n1));
  fprintf(fvtk, "</DataArray>\n");
  fprintf(fvtk,
          "<DataArray type=\"%s\" Name=\"connectivity\" Format=\"ascii\">\n",
          tinto);
  /* for(k=0;k<ns;k++){ */
  /*   kn1=k*n1; */
  ...
  fprintf(fvtk, "</DataArray>\n");
  fprintf(fvtk, "<DataArray type=\"%s\" Name=\"types\" Format=\"ascii\">",
          tinto);
  for (k = 1; k <= ns; k++) fprintf(fvtk, " %lld ", (long long)tcell);
  fprintf(fvtk, "</DataArray>\n");
  fprintf(fvtk, "</Cells>\n");
  //
  INT arrays;
  for (arrays = 0; arrays < vdata->nipt; ++arrays) {
    ...
    fprintf(fvtk, "<DataArray type=\"%s\" Name=\"%s\" Format=\"ascii\">", tinto,
            vdata->names_ipt[arrays]);
    for (k = 0; k < nv; k++)
  ...
  for (arrays = 0; arrays < vdata->ndpt; ++arrays) {
    ...
    fprintf(fvtk, "<PointData Scalars=\"scalars\">\n");
    fprintf(fvtk, "<DataArray type=\"%s\" Name=\"%s\" Format=\"ascii\">",
            tfloat, vdata->names_dpt[arrays]);
    for (k = 0; k < nv; k++) fprintf(fvtk, " %.8f ", vdata->dpt[arrays][k]);
    fprintf(fvtk, "</DataArray>\n");
  }
  fprintf(fvtk, "</PointData>\n");
  /**/
  ...
  for (arrays = 0; arrays < vdata->ndcell; ++arrays) {
    fprintf(fvtk, "<DataArray type=\"%s\" Name=\"%s\" Format=\"ascii\">",
            tfloat, vdata->names_dcell[arrays]);
    for (k = 0; k < ns; k++) fprintf(fvtk, " %.8f ", vdata->dcell[arrays][k]);
    fprintf(fvtk, "</DataArray>\n");
  }
