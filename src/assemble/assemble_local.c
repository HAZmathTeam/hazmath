...
/******************************************************************************************************/
void assemble_DuDv_local(REAL* ALoc,fespace *FE,trimesh *mesh,qcoordinates *cq,INT *dof_on_elm,INT *v_on_elm,INT elm) 
{
	
  /* Computes the local stiffness matrix for <Du,Dv> = <f,v> bilinear form using various element types
  ...
  if(FE->FEtype>0) { // PX elements
	...
	for (trial=0; trial<FE->dof_per_elm; trial++) {
	  kij = (dphix[test]*dphix[trial] + dphiy[test]*dphiy[trial]);
	...
	for (trial=0; trial<FE->dof_per_elm; trial++) {
	  if(dim==2) {
	    kij = dphix[test]*dphix[trial];
	  } else if (dim==3) {
	    kij = dphix[test*dim]*dphix[trial*dim] + dphix[test*dim+1]*dphix[trial*dim+1] + dphix[test*dim+2]*dphix[trial*dim+2];
	  } else {
	    baddimension();
	  }
	  ALoc[test*FE->dof_per_elm+trial] = ALoc[test*FE->dof_per_elm+trial] + w*kij;
...
/******************************************************************************************************/
void assemble_DuDvcoeff_local(REAL* ALoc,fespace *FE,trimesh *mesh,qcoordinates *cq,INT *dof_on_elm,INT *v_on_elm,INT elm,void (*coeff)(REAL *,REAL *,REAL),REAL time) 
{
...
/******************************************************************************************************/
void assemble_mass_local(REAL* MLoc,fespace *FE,trimesh *mesh,qcoordinates *cq,INT *dof_on_elm,INT *v_on_elm,INT elm) 
{
  ...
  if(FE->FEtype>0) { // PX elements
	...
	// Loop over Trial Functions (Columns)
	for (trial=0; trial<FE->dof_per_elm; trial++) {
	  kij = (phi[test*dim]*phi[trial*dim] + phi[test*dim+1]*phi[trial*dim+1]);
	  if(dim==3) kij = kij + phi[test*dim+2]*phi[trial*dim+2];
	  MLoc[test*FE->dof_per_elm+trial] = MLoc[test*FE->dof_per_elm+trial] + w*kij;
	}
      }
    }
...
/******************************************************************************************************/
void assemble_masscoeff_local(REAL* MLoc,fespace *FE,trimesh *mesh,qcoordinates *cq,INT *dof_on_elm,INT *v_on_elm,INT elm,void (*coeff)(REAL *,REAL *,REAL),REAL time) 
{
