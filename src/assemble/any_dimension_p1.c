 ...
 *                  REAL *vols,void *wrk);
 *
 * \brief Computes the (d+1 x d) matrix [B^{-T} ;
 * -B^{-T}*ones(dim,1)]=grad(barycentric). Here B is the mapping such that
 * \hat{x} = B^{-1}(x-x_0) it also computes the volume of the simplex, which is
 * det(B)/d_factorial.
 *
 ...
 * \note
 */
SHORT grad_compute(INT dim, REAL factorial, REAL* xs, REAL* grad, void* wrk) 
{
  ...
  // in this setting grad is (dim+1) x dim matrix, one barycentric gradient per
  // row???
 ...
 * \note
 */
void local_sm(REAL* slocal, REAL* grad, const INT dim, const REAL vols) 
{
...
REAL* local_mm(const INT dim) 
{
  ...
  smii = 2e0 * smij;
  REAL* mlocal =
      calloc(dim1 * dim1, sizeof(REAL));  // local matrix stored by rows.
  for (j = 0; j < dim1; j++) {
    for (k = 0; k < dim1; k++) {
      if (k == j) continue;
...
INT assemble_p1(scomplex* sc, dCSRmat* A, dCSRmat* M) 
{
  ...
  //  fprintf(stdout,"\nnum_simplices=%d ;
  //  num_vertices=%d",ns,nv);fflush(stdout);
  // to compute the volume and to grab the local coordinates of the vertices in
  // the simplex we need some work space
  ...
  REAL* xs = calloc(
      dim * dim1,
      sizeof(REAL));  // for the local coordinates of vertices of a simplex;
  void* wrk = calloc(
      dim1 * dim1,
      sizeof(
          REAL));  // this is used in every simplex but is allocated only once.
  ////////////////////// ASSEMBLY BEGINS HERE:
  // create a block diagonal mass and stiffness matrices with the local matrices
  // on the diagonal
  dCSRmat* m_dg = malloc(sizeof(dCSRmat));
  m_dg[0] = dcsr_create(ns * dim1, ns * dim1, ns * dim1 * dim1);
  /*
    Here we use the fact that we can use the asme IA and JA for the stiffness
    and mass matrices.
  */
  ...
  for (i = 0; i < ns; ++i) {
    ...
    local_coords(dim, xs, &sc->nodes[idim1],
                 sc->x);  // sc->nodes[idim1:idim1+dim]
                          // point to global
                          // vertex numbers in
                          // element i
    // compute gradients
  ...
  P->nnz = nnzp;
  //  fprintf(stdout,"\n P-check nonzeroes(a=b):%d
  //  %d\n",P->nnz,P->IA[m_dg->row]);fflush(stdout); So now we have P and m_dg,
  //  the assembly is P^T*m_dg*P;
