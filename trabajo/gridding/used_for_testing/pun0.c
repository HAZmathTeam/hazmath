...
{
  /* 
     xmacro[] are coordinates of 2^d vertices of a n-dimensional
     parallelepiped. The array x[] is input and output. on input it is
     the the parallelepiped with corners xog and xng; on output it is
     the mapping that takes this rectangle to the parallelepiped
     defined by xmacro[]
  */
  INT i,j,k;
  unsigned int *bits;
...
{  
  ...
  for (k=0;k<nv;k++){
    i=k*dim; j=i+1;
    if(x[i] > xmiddle) {
      xhat=(x[i]-xmiddle)/(dx2);
      yhat=(x[j]-gloc->xog[1])/(dy2);
      //move point:
      phi1 = (1.-xhat)*(1.-yhat);
      phi2 = (xhat)*(1.-yhat);
      phi3 = (1.-xhat)*(yhat);
      phi4 = (xhat)*(yhat);
      if(ib[k]==4) ib[k]=15; //this makes the slanted boundary with
			     //this vertex code.
      x[i] =							\
	xmiddle*phi1+gloc->xng[0]*phi2+xmiddle*phi3+gloc->xng[0]*phi4;	      
      x[j] =								\
	gloc->xog[1]*phi1+gloc->xog[1]*phi2+gloc->xng[1]*phi3+ymiddle*phi4;
