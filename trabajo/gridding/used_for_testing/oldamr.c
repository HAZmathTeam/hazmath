  ...
  case 1: case 2: case 3:
    ...
    */
    if(!sc->level){
      ...
      all=(void *)malloc(4*sizeof(dvector));
      solfem=(dvector *)all;
    ...
    */
    ...
    // this is our solve. This is not a real solve it is only for
    // illustration.
...
void exmpl_estimate(INT amr_ref_type, scomplex *sc, void *all){
  INT j,dim=sc->n;
  dvector *solfem=NULL, *exact=NULL;
  switch(amr_ref_type){
  case 1: case 2: case 3:
    // given a reference point (an "exact solution") this computes the
    // errors. The numerical solution should be the first dvector in all;
    solfem=(dvector *)all;// this is already obtained in the solve phase;
    errs=solfem+1;
    errs[0]=dvec_create(sc->ns);
    exact=errs+1;
    exact[0]=dvec_create(sc->ns);
    memset(exact->val,exact->row*sizeof(REAL));
    for(j=0;j<sc->ns;j++){
      if(sc->child0[j]<0 || sc->childn[j]<0 || sc->gen[j] != sc->level){
	continue;
      }
      errs->val[j]=fabs(solfem->val[j]-exact[j]);
    }
    break;
  default:
