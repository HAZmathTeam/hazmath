...
int main (int argc, char* argv[])
{
  ...
  */
  ...
  INT uzdof;
  INT udof = uxdof+uydof;
  INT pdof = FE_p.ndof;
  INT ndof = udof + pdof;
  INT nun = dim+1; // Number of scalar unknowns
  INT nspaces = dim+1; // Number of FE spaces
  if(dim==3) {
    uzdof = FE_uz.ndof;
    udof += uzdof;
    ndof += uzdof;
  ...
  printf("\n\t--- Element Type ---\n");
  printf("Displacement Element Type = %d\tPressure Element Type = %d\n",order_u,order_p);
  printf("\n\t--- Degrees of Freedom ---\n");
  ...
  */
  ...
  bdcsr_free( &A );
  dcsr_free( &Mp);
  for(i=0;i<dim+1;i++) dcsr_free( &A_diag[i] );
