function [ cover ] = generate_cover_min( AG )
...
% Based on minimal weights  -- Xiaozhe
...
path_ref = cell(m,2); % for ith row, first cell is the all the vertices in the path,
...
AG = triu(AG, 1); %if the graph is undirected
[row,col,val] = find(AG); 
