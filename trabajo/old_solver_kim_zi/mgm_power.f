...
C=====================================================================
      subroutine mg_tr(m,iao,jao,idiro,r,sol,rhs,ao,
     >     n,nnz,maxit,tol,iexact) 
C=====================================================================
      implicit real*8(a-h,o-z)
      dimension m(1)
...
C
C...     Compute at = A^t.
         call aat(m(ia(i)),m(ja(i)),r(ka(i)),nd(i),nd(i),
     >        m(iat),m(jat),r(kat))
C           
C...     Move A^t --> A. 
         call icopyv(m(iat),m(ia(i)),nd(i) + 1)
         call icopyv(m(jat),m(ja(i)),nz(i))
         call copyv(r(kat),r(ka(i)),nz(i))
C
C...     To determine whether Tarjan's ordering is considered or not.
C...     If ordering is considered, then set NORDER = 1.
C
...
C=====================================================================
      subroutine mg_tr1(m,iao,jao,idiro,r,sol,rhs,ao,
     >     n,nnz,maxit,tol,iexact) 
C=====================================================================
      implicit real*8(a-h,o-z)
      dimension m(1)
...
C...     Iterator B using an MG-cycle, i.e., u = B*b = B(rhs - A*sol).
C
         call premg_star(m(1),r(1),lasti,lastr)
C
C...     New solution:  sol = sol + u = sol + B(rhs - A*sol).
         call uupluv(sol,r(kuf),n)
C
...
C=====================================================================
      subroutine mg_star(m,iao,jao,idiro,r,sol,rhs,ao,
     >     n,nnz,maxit,tol,iexact,istar) 
C=====================================================================
      implicit real*8(a-h,o-z)
      dimension m(1)
      dimension r(1)
...
cc         write(*,*) '   METHOD :  MG or MG_normal : V-cycle '
cc      else if (icycle .eq. 2) then
cc         write(*,*) '   METHOD :  MG or MG_normal : Backslash cycle '
cc      else if (icycle .eq. 3) then
cc         write(*,*) '   METHOD :  MG or MG_normal : Variable V-cycle '
cc      end if
ccC
cc      write(*,*) '     Residual :        r := b-Au; '
cc      write(*,'(a,a,e12.4)')
cc     >     '      Convergence :     ||r||_0/||b||_0  and/or',
cc     >     '  ||u - soln||_0/||soln||_0 <', tol
cc      write(*,'(a,2(5X,i3))') '      Coarse and fine levels : ',lc,lf
cc      write(*,'(a,a)') ' -----------------------------',
cc     >           '--------------------------------------------------'
cc      write(*,*) 'Iteration  ||r||/||b||      ||r||  ',
cc     >                  ' ||u-soln||/||soln||   ||Br||   |Br||/||Bb||'
cc      write(*,'(a,a)') ' -----------------------------',
cc     >           '--------------------------------------------------'
C
C...  Compute the L^2 norm of the computed exact solution if it is given.
...
C
      kk = 0
cc      write(*,'(2X,i5,2X,3(3X,e11.4))') 
cc     >     kk,1.d0,err_res0,1.d0
C
      if (ipre_normal .eq. 0) go to 155
C
      kbta  = lastr
...
cc      write(*,*), ' Iteration limit exceeded:', maxit
 333  continue
C
cc      write(*,*) ' tol ' , tol, ' err ' , err_rel
      if (istar .eq. 1) then
C...     Compute b = (I-(A^t)*B^t)*rhs = rhs - A^t*sol.
         call abyvam(rhs,m(ia(lf)),m(ja(lf)),r(ka(lf)),sol,n,r(kbf))
         call copyv(r(kbf),sol,n)
      end if
C
      niter = min(kk, maxit)
cc      write(*,'(2X,i5,2X,3(3X,e11.4))')
cc     >     niter,err_relres,err_res,err_rel
C
cc      write(*,'(a,a)') ' =============================',
cc     >      '=================================================='
C
...
C=====================================================================
      subroutine premg_star(m,r,lasti,lastr)
C=====================================================================
      implicit real*8(a-h,o-z)
      dimension m(1)
      dimension r(1)
