...
# modified by Ludmil 20260317 with the help of Claude (Anthropic)
#
# Extracts C function prototypes from source files to generate hazmath.h.
# Skips static/extern functions and lines not starting with known types.
# Handles three cases for function definitions:
#   1. One-liners:  REAL foo(args) { body; }
#   2. Opening { on same line:  REAL foo(args) {
#   3. Signature only on line:  REAL foo(args)  (with { on next line)
# Multi-line signatures (args spanning lines) are accumulated via inheader.
