/*! \file ReactionDiffusionSystem.h
 *
 *
 * \brief This contains all the local assembly routine for the diffusion matrix of React-diff eq
 *
 */
        ...
        for (trial=0; trial<FE->dof_per_elm; trial++) {
          ...
          for(idim=0;idim<dim;idim++){
			  ...
			  for (jdim = 0; jdim < dim; jdim++){
				...
				aij +=  coeff_val[dim*idim+jdim]*(FE->dphi[test*dim+jdim]);	//mat-vec multipliation
			  }
