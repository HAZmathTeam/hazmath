/*! \file examples/reaction_diffusion/reaction_diffusion.c
*
*  Created by JaCasey Cavanaugh 2018/10/28
...
int main (int argc, char* argv[])
{
  ...
  if(FE.FEtype>0 && FE.FEtype<10) { // PX
    if(dim==1) {
      assemble_global(&A,&b,assemble_local_diffusion,&FE,&mesh,cq,rhs_1D,pde_coeff,0.0);
    } else if(dim==2) {
      assemble_global(&A,&b,assemble_local_diffusion,&FE,&mesh,cq,rhs_2D,pde_coeff,0.0);
    } else if(dim==3) {
      assemble_global(&A,&b,assemble_local_diffusion,&FE,&mesh,cq,rhs_3D,pde_coeff,0.0);
    } else {
      status = ERROR_DIM;
  ...
  // Again this depends on dimension for different examples
