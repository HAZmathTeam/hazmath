...
INT main(int argc,char *argv[])
{
  ...
  fprintf(stdout,"\n%% AUTO GENERATED\n");
  fprintf(stdout,"\n%%%%EXAMPLE(fractional):\n");
  fprintf(stdout,"\nxmin=%.8e;xmax=%.8e;npts=%d;h=(xmax-xmin)/(npts-1);\n",xmin_in,xmax_in,4097);
  ...
  fprintf(stdout,"\n%%===============================================%%\n");
  for(i=0;i<m;i++)fprintf(stdout,"\nres(%d)=%.16e + im*(%.16e);",i+1,*(rpzwf[0]+i),*(rpzwf[1]+i));
  fprintf(stdout,"\n");
  for(i=0;i<m-1;i++) fprintf(stdout,"\npol(%d)=%.16e + im*(%.16e);",i+1,*(rpzwf[2]+i),*(rpzwf[3]+i));
  fprintf(stdout,"\n");
  for(i=0;i<m;i++) fprintf(stdout,"\nz(%d)=%.16e;",i+1,*(rpzwf[4]+i));
  fprintf(stdout,"\n");
  for(i=0;i<m;i++)fprintf(stdout,"\nw(%d)=%.16e;",i+1,*(rpzwf[5]+i));
  fprintf(stdout,"\n");
  for(i=0;i<m;i++)fprintf(stdout,"\nf(%d)=%.16e;",i+1,*(rpzwf[6]+i));
  fprintf(stdout,"\n");  
