...
cc       parameter (nlast = 52 000 000, nrlast = 55 000 000) !level = 10
       parameter (nlast = 140 000 000, nrlast = 99 000 000) !level = 11
...
C 
      ...
      tol     = 1.d-8
C
cc      write(*,1000) lread,lpde,lmethod,nsub,lc,ich(23)
...
C
cc      call outmat1(m(iac),m(jac),r(krac),nc,nnzc)
cc      stop 111
C
...
C
      ...
      else if (ichoice_rhs .eq. 0) then
         call read_soln(lf,r(ku_exact),n,lread)
      else
         call nullv(r(ku_exact),n)
      end if
C
      kb     = ku_exact + n
      kfree  = kb + n
...
C
cc         write(*,*) 'bbbb', (r(kb+i-1), i=1,nc)

         call sgauss(m(iac),m(jac),r(krac),r(kb),m(mfree1),r(kfree1),nc)

         write(*,*) 'Gaussian Elimination is done'
...
cc         write(*,*) 'bbbb', (r(ksol+i-1), i=1,n)
C
...
cc         call l2norm(r(kerr),snorm,n,hhhh)
     ...
     >        r(kx),r(ky))
         h1norm = dsqrt(sl2norm*sl2norm + h1snorm*h1snorm)
C
cc         write(*,'(a,f6.1,4(e14.4))') 
cc     >        ' No.iter     L_2      H_1: ',kiter-0.5,sl2norm,h1norm
C
         write(*,'(f6.1,2(e14.4))') kiter-0.5, sl2norm, h1norm
C
...
C
CCCCCCCCCCCCCCCCCCCCCCCCCC  Temporary CCCCCCCCCCCCCCCCCCCCCCCCCCCC
         write(*,*) (i,'*',r(ksol+i), i = 1500,1000000,100000)
C
         smin = r(ksol)
         smax = smin
C     
...
C****************************************************************************
      iexact = 1
      if (iexact .eq. 1) then
         call write_soln(lf,r(ksol),n,lread)
      end if
C
      if (n .le. 65*65) then
