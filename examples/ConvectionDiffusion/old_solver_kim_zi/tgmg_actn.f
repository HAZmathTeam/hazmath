...
C=====================================================================
      ...
      parameter (iter_tg_max = 14)
cc      parameter (nlast = 2 000 000, nrlast = 6 000 000) !level = 8
cc      parameter (nlast = 10 000 000, nrlast =20 000 000) !level = 9
      parameter (nlast = 40 000 000, nrlast = 55 000 000) !level = 10
cc      parameter (nlast = 140 000 000, nrlast = 100 000 000) !level = 11
cc      parameter (nlast = 120 000 000, nrlast = 90 000 000) !level = 11
...
cc      dimension m(nlast), r(nrlast)
      ...
      dimension ku(lmx),kb(lmx),ka(lmx),krhs(lmx),ksol(lmx)
C---------------------------------------------------------------------
...
C...
C...  NOTE: This is a variant of the program TGMG. Here MG V-cycles are 
C...    performed using only actions without storing stiffness matrices.
C...  
C...  Parameters:
...
C
      ...
      lcmg   = 1
C
      call mg_actn(r(1),r(ku(2)),r(kb(2)),lastr,nd(2),lfmg,lcmg,
     >     icycle,iprsm,ipssm,nprsm,npssm,lsolv,maxit,tol)
C
cc      call mg_s(m(1),ia(2),ja(2),idir(2),lasti,
cc     >     r(1),ku(2),kb(2),kal(2),lastr,
cc     >     nd(2),n1(2),n2(2),nz(2),nel(2),lfmg,lcmg,tol) 
C
...
C
         if (ich_sol .eq. 1) then
            ...
            call nullv(r(ksol_exact),nd(lvl))
         end if
C
...
C
         ...
         kwk2 = kerr + nd(lvl)
C
C...     Compute sl2norm2 = err*M*err, where M is a mass matrix. 
         call abyvg_actn_more(multl(lvl),nd(lvl),r(kerr),
     >        r(kwk1),r(kwk2),6,0.d0,0.d0,1.d0)
         call scpro(r(kerr),r(kwk2),sl2norm2,nd(lvl))
         sl2norm = dsqrt(sl2norm2)
C
C...     Compute h1snorm2 = err*A*err, where A is a Laplacian. 
         call abyvg_actn_more(multl(lvl),nd(lvl),r(kerr),
     >        r(kwk1),r(kwk2),1,0.d0,0.d0,0.d0)
         call scpro(r(kerr),r(kwk2),h1snorm2,nd(lvl))
cc         h1snorm = dsqrt(h1snorm2)
         h1norm = dsqrt(sl2norm2 + h1snorm2)
C
cc         call l2_h1s_norm(0,r(kerr),nd(lvl),sl2norm,nel(lvl),jdf,
cc     >        m(ief),m(jef),r(kxf),r(kyf))
cc         call l2_h1s_norm(1,r(kerr),nd(lvl),h1snorm,nel(lvl),jdf,
cc     >        m(ief),m(jef),r(kxf),r(kyf))
cc         h1norm = dsqrt(sl2norm*sl2norm + h1snorm*h1snorm)
C
         iter_tg = iter_tg + 1
C     
         write(*,'(a,i6,2(e14.4))') 
...
cc      tol  = 1.d-6
      ...
      lcmg = 1
C
      call mg_actn(r(1),r(ku(mk)),r(kb(mk)),lastr,nd(mk),lfmg,lcmg,
     >     icycle,iprsm,ipssm,nprsm,npssm,lsolv,maxit,tol)
C
cc      call mg_s(m(1),ia(mk),ja(mk),idir(mk),lasti,
cc     >     r(1),ku(mk),kb(mk),kal(mk),lastr,
cc     >     nd(mk),n1(mk),n2(mk),nz(mk),nel(mk),lfmg,lcmg,tol) 
C
C...  Update the solution: sol = sol + u.
