...
{
  bool bEG = true;//false;//false;//false;//true;//false;//true;//false;//true;
	...
	if(bEG){
	  ...
	  for (test=0; test<FE->var_spaces[dim]->dof_per_elm;test++){
	    ...
	    for (trial=0; trial<FE->var_spaces[1]->dof_per_elm;trial++){
	      ...
	      // NEW SLEE : elasticity div part
	      // u2-v1 block : <dy(u2),dx(v1)> 
	      kij += 0.5 *lambda*neighbor_basis_1_dphi[trial*dim+1] * finrm[0] *  (qx_face[0]- barycenter_neighbor->x[0]);	
	      kij += 0.5 *lambda*neighbor_basis_1_dphi[trial*dim+1] * finrm[1] *  (qx_face[1]- barycenter_neighbor->y[0]);
...

{
  ...
  exit(0);
  */
  bool bEG = true;// false;//true;//false;//true;//false;//true;//false;//true;//false;//true;// true;//false;//true;//false;//true;//false;//true;//true;//true;//false;//true;//false;//false;//true;//false;//true;//false;//true;//false;
  ///////////
...
int main (int argc, char* argv[])
{
  ...
  */ 
  ...
  for(int tmp=0; tmp<total_num_cycle; ++tmp)
    ...
    for(int tmp=0; tmp<total_num_cycle; ++tmp)
    {
      printf("%d &  %f & %f \\\\ \\hline \n",  dof_per_cycle_EG[tmp],
	     H1_energy_EG_error_per_cycle[tmp], H1_energy_EG_conv_rate_per_cycle[tmp] ); 
    }
